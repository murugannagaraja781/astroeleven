...
function getFullVimshottariDashaTree(moonLongitude, birthDate) {
    // 1. Calculate Mahadashas (Level 1) in ms
    const nakSpan = 360 / 27;
    const nakIndex = Math.floor(moonLongitude / nakSpan);
    const lordIndex = nakIndex % 9;
    const currentLord = DASHA_SEQUENCE[lordIndex];
    ...
    let currentDateMs = birthDate.minus({ days: daysElapsed }).toMillis();
    const mahadashas = [];
    
    for (let i = 0; i < 9; i++) {
        const idx = (lordIndex + i) % 9;
        const lord = DASHA_SEQUENCE[idx];
        const startDateMs = currentDateMs;
        const endDateMs = currentDateMs + Math.floor(lord.years * 365.2425 * 24 * 60 * 60 * 1000);
    ...
    // 2. Helper to get subperiods in ms
    function getSubPeriodsMs(parentStartMs, parentEndMs, parentLord, level) {
        const totalDuration = parentEndMs - parentStartMs;
        const lIndex = DASHA_SEQUENCE.findIndex(d => d.lord === parentLord);
        if (lIndex === -1) return [];
        
        const timeline = [];
        let currentMs = parentStartMs;
        
        for (let i = 0; i < 9; i++) {
            const idx = (lIndex + i) % 9;
            const lord = DASHA_SEQUENCE[idx];
            const subDuration = (lord.years / 120) * totalDuration;
            const endDateMs = currentMs + subDuration;
    ...
    // 3. Nested mapping in pure milliseconds numbers, only toISO() at the final return
    return mahadashas.map(md => {
        const bhuktis = getSubPeriodsMs(md.start, md.end, md.lord, 1);
        return {
            lord: md.lord,
            start: DateTime.fromMillis(md.start).toISO(),
            end: DateTime.fromMillis(md.end).toISO(),
            level: 1,
            subPeriods: bhuktis.map(bh => {
                const antaras = getSubPeriodsMs(bh.start, bh.end, bh.lord, 2);
                return {
                    lord: bh.lord,
                    start: DateTime.fromMillis(bh.start).toISO(),
                    end: DateTime.fromMillis(bh.end).toISO(),
                    level: 2,
                    subPeriods: antaras.map(an => {
                        const pratyantaras = getSubPeriodsMs(an.start, an.end, an.lord, 3);
                        return {
                            lord: an.lord,
                            start: DateTime.fromMillis(an.start).toISO(),
                            end: DateTime.fromMillis(an.end).toISO(),
                            level: 3,
                            subPeriods: pratyantaras.map(pr => {
                                return {
                                    lord: pr.lord,
                                    start: DateTime.fromMillis(pr.start).toISO(),
                                    end: DateTime.fromMillis(pr.end).toISO(),
                                    level: 4
                                };
                            })
                        };
